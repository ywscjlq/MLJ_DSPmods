...
public class MySideSlider : MonoBehaviour {
    ...
    public static MySideSlider CreateSlider(float x, float y, RectTransform parent, float value, float minValue,
        float maxValue, string format = "G", float width = 0f, float textWidth = 0f) {
        return CreateSlider(x, y, parent, width, textWidth).WithLabelFormat(format).WithMinMaxValue(minValue, maxValue)
            .WithValue(value);
    }

    public static MySideSlider CreateSlider(float x, float y, RectTransform parent, float width = 0f,
        float textWidth = 0f) {
        ...
        if (sl.slider.transform is RectTransform rectTrans) {
            rectTrans.localPosition = new(width, rectTrans.localPosition.y, rectTrans.localPosition.z);
            rectTrans.sizeDelta = new(width, rectTrans.sizeDelta.y);
        }
        sl.Value = 0f;
        ...
        if (sl.labelText.transform is RectTransform rectTrans2) {
            if (textWidth > 0f) {
                rectTrans2.sizeDelta = new(textWidth, rectTrans2.sizeDelta.y);
            } else {
                rectTrans2.sizeDelta = new(-textWidth, rectTrans2.sizeDelta.y);
            }
            rectTrans2.pivot = new(0f, 1f);
            rectTrans2.localPosition = new(textWidth <= 0f ? width + 10f : width, rectTrans2.localPosition.y,
