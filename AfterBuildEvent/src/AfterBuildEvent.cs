...
static class AfterBuildEvent {
    ...
    private static void UpdateModsThenStart(bool automationMode = false, string[] args = null) {
        ...
        foreach (var dirInfo in new DirectoryInfo(SolutionDir).GetDirectories()) {
            ...
            if (projectName == "GetDspData") {
                ...
                //fe
                string originFEAssets = string.IsNullOrEmpty(PathConfig.FEAssetsDir) ? "" : $@"{PathConfig.FEAssetsDir}\fe";
                string projectFEAssets = $@"{SolutionDir}\FractionateEverything\Assets\fe";
                if (!string.IsNullOrEmpty(originFEAssets) && File.Exists(originFEAssets)) {
    ...
    private static void PrepareR2Doorstop() {
        try {
            string winhttpSrc = $@"{R2ProfileDir}\winhttp.dll";
            string doorstopSrc = $@"{R2ProfileDir}\doorstop_config.ini";
            if (!File.Exists(winhttpSrc)) {
                Console.WriteLine($"跳过 doorstop 配置：缺少 {winhttpSrc}");
                return;
            }
            File.Copy(winhttpSrc, $@"{DSPGameDir}\winhttp.dll", true);
            if (!File.Exists(doorstopSrc)) {
                Console.WriteLine($"跳过 doorstop_config：缺少 {doorstopSrc}");
                return;
            }
            string doorstop_config = $@"{DSPGameDir}\doorstop_config.ini";
            File.Copy(doorstopSrc, doorstop_config, true);
            ...
            Console.WriteLine($"跳过 doorstop 配置（异常）：{ex.Message}");
        }
    }
    ...
    private static void PublizeDll(CmdProcess cmd, string dllPath, string targetPath) {
        ...
        while (true) {
            try {
                ...
                if (++retry >= maxRetry) {
                    Console.Error.WriteLine($"复制失败(已重试{maxRetry}次): {ex.Message}");
                    return;
                }
                Thread.Sleep(100);
            }
